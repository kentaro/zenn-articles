...
sync_session() {
    ...
    # Get today's start time in UTC (for JST: today 00:00 JST = yesterday 15:00 UTC)
    ...
    # Extract conversation for today only (comparing UTC timestamps with JST day boundary), filter noise
    local content=$(cat "$session_file" | jq -r --arg today_start "$TODAY_START_UTC" '
    ...
    if [ -n "$content" ]; then
        # Preserve camera watch entries before overwriting (deduplicated)
        local camera_entries=""
        if [ -f "$OUTPUT_FILE" ]; then
            camera_entries=$(grep "^\*\*📷 監視AI\*\*" "$OUTPUT_FILE" 2>/dev/null | sort -u || true)
        fi
        ...
        # Append preserved camera entries
        if [ -n "$camera_entries" ]; then
            echo "" >> "$OUTPUT_FILE"
            echo "$camera_entries" >> "$OUTPUT_FILE"
        fi
